...
% Compute voxel-wise correlations for the interacting dyads in Game 2.
...
out_dir = fullfile(base_dir, 'r_results', ...
    'paired_whole_run_residual_rmMotionCSFWM');
subjects_file = fullfile(base_dir, 'subjects.txt');
pairs_file = fullfile(base_dir, 'pairs.txt');
...
pairs_raw = importdata(pairs_file);
...
for i = 1:numel(pairs)
    pair_id = pairs{i};
    if numel(pair_id) ~= 10
        error('Invalid dyad ID in %s: %s', pairs_file, pair_id);
    end
    ...
    if size(sub1_tc, 1) ~= size(sub2_tc, 1)
        error('Voxel-count mismatch for dyad %s.', pair_id);
    end
    if ~isequal(sub1_tc(:, 1:3), sub2_tc(:, 1:3))
        error('Voxel-coordinate mismatch for dyad %s.', pair_id);
    end

    % Columns 1-3 are voxel coordinates. Use the common temporal overlap.
    last_col = min(size(sub1_tc, 2), size(sub2_tc, 2));
    if last_col < 5
        error('Fewer than two time points for dyad %s.', pair_id);
    end
    ...
    fprintf('Computing interacting dyad %s (%d/%d)\n', ...
        pair_id, i, numel(pairs));
